...
public class RageSword extends SwordItem implements DamageTrackingWeapon {
    private static final String DAMAGE_KEY = "DamageDealt";
    public static final int DAMAGE_THRESHOLD = 20; // 10 hearts

    public RageSword(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            ...
            if (damageDealt >= DAMAGE_THRESHOLD) {
                user.addStatusEffect(new StatusEffectInstance(ModEffects.RAGEEFFECT,100,1));
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(Text.translatable("item.azara.rage_sword.desc"));
